...
fn main() -> std::io::Result<()> {
    ...
    let is_git_info_available = git_cmd().arg("status").status()
    ...
    let data = VersionData {
        ...
        branch: if is_git_info_available { get_branch_name().unwrap_or("NONE".to_string()) } else { "NONE".to_string() },
        commit: if is_git_info_available { get_commit_hash() } else { "NONE".to_string() },
    ...
    file.write(serde_json::to_string::<VersionData>(&data).unwrap().as_bytes()).unwrap();
